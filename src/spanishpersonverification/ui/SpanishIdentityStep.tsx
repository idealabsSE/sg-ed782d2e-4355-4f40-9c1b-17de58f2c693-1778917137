...
interface SpanishVerificationData {
  role: UserRole;
...
export function SpanishIdentityStep() {
  const { t } = useTranslation();
  const router = useRouter();
  const { toast } = useToast();
  const [currentStep, setCurrentStep] = useState<VerificationStep>("role");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [data, setData] = useState<SpanishVerificationData>({
    role: "",
    documentType: "",
    idNumber: "",
    uploadedFiles: [],
  ...
  const handleDocumentTypeChange = (type: SpanishDocumentType) => {
