...
interface SwedishVerificationData {
  role: UserRole;
  documentType: SwedishDocumentType;
  idNumber: string;
  uploadedFiles: File[];
}

export function SwedishIdentityStep() {
  const { t } = useTranslation();
  const router = useRouter();
  const { toast } = useToast();
  const [currentStep, setCurrentStep] = useState<VerificationStep>("role");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [data, setData] = useState<SwedishVerificationData>({
    role: "",
    documentType: "",
    idNumber: "",
    uploadedFiles: [],
  ...
  const handleDocumentTypeChange = (type: SwedishDocumentType) => {
